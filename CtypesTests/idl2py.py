...
def PointerLevels (argument):
    ...
    if argument['kind'] == 'interface':
        return argument['pointer']-1
    return argument['pointer']


def CtypesFor (argument, levels):
    '''ctypes type of a described argument with the given number of pointer levels.'''
    if argument['kind'] == 'interface':
...
def GenerateInterface (interface):
    ...
    for method in interface['methods']:
        ...
        params = ''  # the Python method's parameters
        types = ''   # ctypes type of every argument
        values = ''  # what is passed on to the vtable call
        for argument in arguments:
            if types:
                types += ', '
            types += CtypesFor(argument, PointerLevels(argument))
            if argument is retval:
                values += ', ctypes.byref('+argument['name']+')'
            else:
                params += ', '+argument['name']
                values += ', '+argument['name']

        out += '\n    def '+method['name']+' (self'+params+'):\n'
        if retval:
            out += '        '+retval['name']+' = '+CtypesFor(retval, PointerLevels(retval)-1)+'()\n'

        out += '        hr = comruntime.Call(self.pointer, '+str(method['slot'])+', ['+types+']'+values+')\n'
        out += "        comruntime.Check(hr, '"+interface['name']+'.'+method['name']+"')\n"

        if retval:
            if retval['kind'] == 'interface':
...
def GenerateImplementation (interface, iids):
    ...
    for method in interface['methods']:
        ...
        retval = 'None'
        if arguments and 'retval' in arguments[-1]['direction']:
            retval = arguments[-1]['name']

        types = 'ctypes.c_int32, ctypes.c_void_p'  # return value and 'this'
        names = ''   # every argument, as the vtable passes them
        passed = ''  # the [in] arguments, handed to the handler
        for argument in arguments:
            types += ', '+CtypesFor(argument, PointerLevels(argument))
            names += ', '+argument['name']
            if argument['name'] != retval:
                if passed:
                    passed += ', '
                passed += argument['name']

        out += '        ctypes.CFUNCTYPE('+types+')(\n'
        out += '            lambda this'+names+': comruntime.Returned(\n'
        out += '                '+retval+', handler.'+method['name']+'('+passed+'))),\n'
...
def GenerateBindings (metadata):
    ...
    for interface in metadata['interfaces']:
        iid_of[interface['name']] = interface['iid']
        base_of[interface['name']] = interface['base']
    ...
    for interface in metadata['interfaces']:
        ...
        while name in iid_of:
            iids.append(iid_of[name])
            name = base_of[name]
        iids.append(comruntime.IID_IUnknown)

        out += GenerateInterface(interface)
        out += GenerateImplementation(interface, iids)
