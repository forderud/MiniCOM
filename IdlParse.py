...
def ParseInterfaces (source):
    ...
    def ReplaceFun2 (match):
        ...
        index = source.count(': public', 0, match.start())-1
        if index >= 0:
...
def DescribeArgument (argument, interfaces):
    ...
    if directions:
        direction = directions[0].split(',')

    declaration = re.sub('/\\*\\[.*?\\]\\*/', '', argument)
    declaration = declaration.replace('const', '').strip()
    ...
    kind = 'scalar'
    if type_name in interfaces or type_name == 'IUnknown':
        kind = 'interface'
    ...
    if array:
        described['array'] = 0 # size unknown
        if array.group(1):
            described['array'] = int(array.group(1))
    return described
...
def GenerateMetadata (definitions, uuids, coclasses):
    ...
    methods_of = {}
    base_of = {}
    for name, base, methods in definitions:
    ...
    def AllMethods (name):
        ...
        base = base_of[name]
        if base == 'IUnknown':
            return methods_of[name]
        return AllMethods(base)+methods_of[name]
    ...
    for name, base, _ in definitions:
        ...
        methods = []
        slot = 3 # QueryInterface, AddRef and Release occupy the first three slots
        for method, arglist in AllMethods(name):
            arguments = []
            for argument in SplitParameters(arglist):
                arguments.append(DescribeArgument(argument, base_of))
    ...
    for name in coclasses:
        classes.append({'name': name, 'clsid': coclasses[name]})
